...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for definable.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal definable sessions
...
definable acp --cwd /path/to/project
```
...
```bash
DEFINABLE_ENABLE_QUESTION_TOOL=1 definable acp
```
...
- **Terminal Integration**: Full terminal support via definable's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | definable acp
```
...
### Mapping to Definable

ACP sessions map cleanly to definable's internal session model:
