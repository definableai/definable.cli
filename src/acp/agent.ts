...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `definable auth login` in the terminal",
        name: "Login with definable",
        id: "definable-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Definable stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const definableProvider = providers.find((p) => p.id === "definable")
    if (definableProvider) {
      if (definableProvider.models["big-pickle"]) {
        return { providerID: "definable", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(definableProvider.models))
      if (best) {
        return {
