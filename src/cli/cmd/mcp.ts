    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in definable.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .definable/ subdirectory too)
  const candidates = [path.join(baseDir, "definable.json"), path.join(baseDir, "definable.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".definable", "definable.json"), path.join(baseDir, ".definable", "definable.jsonc"))
  }
  ...
  // Default to definable.json if none exist
  return candidates[0]
}
  ...
  async handler() {
    await Instance.provide({
      ...
      async fn() {
        ...
        if (preset?.handle) {
          await preset.handle({ configPath, projectConfigPath, addMcpToConfig })
          return
        }
