...
const TUI_SCHEMA_URL = "https://definable.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from definable.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const definable = await definableFiles(input)
  for (const file of definable) {
...
async function definableFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.DEFINABLE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("definable", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "definable")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "definable"))
  }
  if (Flag.DEFINABLE_CONFIG) files.push(Flag.DEFINABLE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "definable"))
