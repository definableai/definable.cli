...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.DEFINABLE_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.DEFINABLE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.DEFINABLE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".definable") && dir !== Flag.DEFINABLE_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old definable.json shape) still get their settings applied.
    const normalized = (() => {
