...
import { fn } from "@definable-ai/util/fn"
  ...
  export async function fromDirectory(directory: string) {
    ...
    const data = await iife(async () => {
      ...
      if (dotgit) {
        ...
        let id = await Filesystem.readText(path.join(dotgit, "definable"))
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.DEFINABLE_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            await Filesystem.write(path.join(dotgit, "definable"), id).catch(() => undefined)
          }
        }
    ...
    if (Flag.DEFINABLE_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
