...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async definable(input) {
      const hasKey = await (async () => {
        const env = Env.all()
        if (input.env.some((item) => env[item])) return true
        if (await Auth.get(input.id)) return true
        const config = await Config.get()
        if (config.provider?.["definable"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from definable.json provider config
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.DEFINABLE_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("definable")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Check if definable provider is available before using it
    const definableProvider = await state().then((state) => state.providers["definable"])
    if (definableProvider && definableProvider.models["gpt-5-nano"]) {
      return getModel("definable", "gpt-5-nano")
    }
