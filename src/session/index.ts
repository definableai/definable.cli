...
export namespace Session {
  ...
  export async function createNext(input: {
    ...
    if (!result.parentID && (Flag.DEFINABLE_AUTO_SHARE || cfg.share === "auto"))
  ...
  export function plan(input: { slug: string; time: { created: number } }) {
    const base = Instance.project.vcs
      ? path.join(Instance.worktree, ".definable", "plans")
      : path.join(Global.Path.data, "plans")
    return path.join(base, [input.time.created, input.slug].join("-") + ".md")
  }

  export function testReport(input: { slug: string; time: { created: number } }) {
    const base = Instance.project.vcs
      ? path.join(Instance.worktree, ".definable", "test-reports")
      : path.join(Global.Path.data, "test-reports")
    return path.join(base, [input.time.created, input.slug].join("-") + ".md")
  }
  ...
  export const getUsage = fn(
    ...
    (input) => {
      ...
      // It looks like Definable's cost calculation assumes all providers return inputTokens the same way Anthropic does (I'm guessing getUsage logic was originally implemented with anthropic), so it's causing incorrect cost calculation for OpenRouter and others.
