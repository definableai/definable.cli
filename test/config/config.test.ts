...
const managedConfigDir = process.env.DEFINABLE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "definable.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "definable.json") {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://definable.ai/config.json",
        model: "test/model",
        username: "testuser",
      })
    },
...
test("ignores legacy tui keys in definable config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://definable.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "definable.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "definable.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .definable directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const definableDir = path.join(dir, ".definable")
      await fs.mkdir(definableDir, { recursive: true })
      const agentDir = path.join(definableDir, "agent")
...
test("loads agents from .definable/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const definableDir = path.join(dir, ".definable")
      await fs.mkdir(definableDir, { recursive: true })

      const agentsDir = path.join(definableDir, "agents")
...
test("loads commands from .definable/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const definableDir = path.join(dir, ".definable")
      await fs.mkdir(definableDir, { recursive: true })

      const commandDir = path.join(definableDir, "command")
...
test("loads commands from .definable/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const definableDir = path.join(dir, ".definable")
      await fs.mkdir(definableDir, { recursive: true })

      const commandsDir = path.join(definableDir, "commands")
...
test("does not try to install dependencies in read-only DEFINABLE_CONFIG_DIR", async () => {
  ...
  const prev = process.env.DEFINABLE_CONFIG_DIR
  process.env.DEFINABLE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.DEFINABLE_CONFIG_DIR
    else process.env.DEFINABLE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable DEFINABLE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "definable.json"),
        JSON.stringify({ $schema: "https://definable.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "definable.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .definable config
      const projectDir = path.join(dir, "project")
      const definableDir = path.join(projectDir, ".definable")
      await fs.mkdir(definableDir, { recursive: true })
      ...
      // Local .definable config with different plugins
      await Filesystem.write(
        path.join(definableDir, "definable.json"),
        JSON.stringify({
          $schema: "https://definable.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .definable config with some overlapping plugins
      await Filesystem.write(
        path.join(definableDir, "definable.json"),
        JSON.stringify({
          $schema: "https://definable.ai/config.json",
...
// Note: preload.ts sets DEFINABLE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .definable config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .definable directory config enables it
      const definableDir = path.join(dir, ".definable")
      await fs.mkdir(definableDir, { recursive: true })
      await Filesystem.write(
        path.join(definableDir, "definable.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/definable")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/definable")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-definable@2.4.3")).toBe("oh-my-definable")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@definable/plugin@2.0.0")).toBe("@definable/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-definable@2.4.3", "file:///project/.definable/plugin/oh-my-definable.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.definable/plugin/oh-my-definable.js")
  })
  ...
  test("local plugin directory overrides global definable.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const definableDir = path.join(projectDir, ".definable")
        const pluginDir = path.join(definableDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("DEFINABLE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["DEFINABLE_DISABLE_PROJECT_CONFIG"]
    process.env["DEFINABLE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["DEFINABLE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["DEFINABLE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .definable/ directories when flag is set", async () => {
    const originalEnv = process.env["DEFINABLE_DISABLE_PROJECT_CONFIG"]
    process.env["DEFINABLE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .definable directory with a command
          const definableDir = path.join(dir, ".definable", "command")
          await fs.mkdir(definableDir, { recursive: true })
          await Filesystem.write(path.join(definableDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .definable should NOT be in directories list
          const hasProjectDefinable = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectDefinable).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["DEFINABLE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["DEFINABLE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["DEFINABLE_CONFIG_DIR"]
      process.env["DEFINABLE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["DEFINABLE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["DEFINABLE_CONFIG_DIR"]
      } else {
        process.env["DEFINABLE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("DEFINABLE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["DEFINABLE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["DEFINABLE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["DEFINABLE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["DEFINABLE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from DEFINABLE_CONFIG_DIR, not project
...
describe("DEFINABLE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in DEFINABLE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["DEFINABLE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["DEFINABLE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://definable.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["DEFINABLE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["DEFINABLE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in DEFINABLE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["DEFINABLE_CONFIG_CONTENT"]
