...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const definableModel = {
      ...openaiModel,
      providerID: "definable",
      api: {
        id: "definable-test",
        url: "https://api.definable.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, definableModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.definable?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.definable?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          definable: { itemId: "msg_definable" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              definable: { itemId: "msg_definable_part" },
    ...
    expect(result[0].providerOptions?.definable?.itemId).toBe("msg_definable")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.definable?.itemId).toBe("msg_definable_part")
